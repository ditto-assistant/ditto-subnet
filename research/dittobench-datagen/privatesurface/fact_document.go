...
Vary sentence structure, voice, register and presentation without typos or fixed boilerplate. Connective prose and incidental non-factual texture are allowed only if they add no state or evidence. Obey each record's min_bytes/max_bytes for the final bound record, leaving margin for token replacement. When interior_facts is true, place ALL binding tokens in the middle 15%-85% of the record; keep opening and closing texture free of facts and tokens. Do not create a question or answer. Return only the JSON object, without fences.`
