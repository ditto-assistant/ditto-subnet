...
//
// POST /generate?seed=<int64>&run_size=<small|medium|full>&bench_version=<2..13> returns the FULL
...
func handleGenerate(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || !protocol.SupportedBenchVersion(version) {
		http.Error(w, "bench_version query param required (supported: "+protocol.SupportedBenchVersionList()+")", http.StatusBadRequest)
		return
	}
