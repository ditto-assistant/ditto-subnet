...
def _neuron_as_float(value: Any, default: float = 0.0) -> float:
    try:
        if value is None:
            return default
        return float(value)
...
def _neuron_as_int(value: Any, default: int = 0) -> int:
    try:
        if value is None:
            return default
        return int(value)
...
class NeuronInfo:
    ...
    Ditto's platform and validators need, including Yuma metagraph fields for
    the public chain surface.
    """
    ...
    def from_pylon(cls, raw: Any, hotkey: str | None = None) -> NeuronInfo:
        ...
        return cls(
            ...
            uid=_neuron_as_int(getattr(raw, "uid", 0)),
            stake=_neuron_as_float(getattr(raw, "stake", 0.0)),
            ...
            incentive=_neuron_as_float(getattr(raw, "incentive", 0.0)),
            dividends=_neuron_as_float(getattr(raw, "dividends", 0.0)),
            trust=_neuron_as_float(getattr(raw, "trust", 0.0)),
            consensus=_neuron_as_float(getattr(raw, "consensus", 0.0)),
            emission=_neuron_as_float(getattr(raw, "emission", 0.0)),
            last_update=_neuron_as_int(getattr(raw, "last_update", 0)),
            validator_trust=_neuron_as_float(getattr(raw, "validator_trust", 0.0)),
        )
