...
reproduce, exactly like the P2 check on ``derive_validator_seed``. With
``block_hash=None`` the encoding is **byte-identical** to the legacy
derivation, so every v<=12 seed family is unchanged.
"""
...
# so an in-flight v12 reign's seeds never move. Mirrored on the platform.
