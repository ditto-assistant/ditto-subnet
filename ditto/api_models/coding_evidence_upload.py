...
from ditto.api_models.coding import CodingContractModel, Sha256
...
class CodingSealedEvidenceUploadCapability(CodingContractModel):
    """One redacted, ticket-bound PUT capability for sealed local evidence."""
...
class _SignedEvidenceRequest(CodingContractModel):
...
class CodingSealedEvidenceFinalization(CodingContractModel):
    """Platform acknowledgement that one object identity is finalized."""
...
def parse_coding_sealed_evidence_upload_capability_json(
    ...
    try:
        return CodingSealedEvidenceUploadCapability.model_validate_json(body)
...
def _validate_signed_url(capability: CodingSealedEvidenceUploadCapability) -> None:
    ...
    if sum(len(values) for values in query.values()) > 64:
        raise ValueError("coding evidence URL has too many query fields")
...
def _signed_expiry(query: dict[str, list[str]]) -> datetime:
    ...
    try:
        ...
        raise ValueError("coding evidence v2 expiry is invalid") from error
