...
def _certification_authority() -> dict[str, object]:
    return {
        "coding_contract_version": 1,
        "weight_eligible": False,
        "grant_id": _GRANT,
...
def test_certification_grant_offer_exchange_and_revocation_are_strict() -> None:
    offer = CodingCertificationInferenceGrantOffer.model_validate(
        {
    ...
    exchange = CodingCertificationInferenceExchangeResponse.model_validate(
        {
    ...
    assert exchange.generation == 1
    revoked = CodingCertificationInferenceRevokeResponse(
    ...
    assert revoked.lease_id == _LEASE
    with pytest.raises(ValidationError):
        CodingCertificationInferenceGrantOffer.model_validate(
            {
                "schema": "dittobench-coding-certification-inference-grant-offer-v1",
                **_certification_authority(),
                "status": "pending",
    ...
    assert message.startswith(b"dittobench-coding-certification-inference-grant:v1:")
