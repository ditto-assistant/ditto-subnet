...
class TestRunBudget:
    ...
    @pytest.mark.parametrize("ttl_minutes", [30, 45, 90, 120, 180])
    def test_the_abort_always_lands_before_the_deadline_with_margin(
        self, ttl_minutes: int
    ) -> None:
        """The latent failure this closes.

        ``_TICKET_TTL`` has already moved 30 -> 45 -> 90 -> 120 -> 180 minutes.
        At 30 and 45 minutes a fixed harness cap can outlive a shortened lease,
        so the harness would still be polling when the ticket died -- guaranteed
        silent expiry. The budget must leave the reporting margin at every TTL,
