...
* the legacy validator-derived lanes (version-bump re-score sweep, contested
  dethrone) draw no fresh seed at a binding version until the ledger carries a
  pinned anchor, and hash that anchor when it does;
...
class TestLegacyLanesWaitForTheLedgerAnchor:
    ...
    async def test_rescore_sweep_defers_without_a_pinned_anchor(self) -> None:
