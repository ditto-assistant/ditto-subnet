...
def test_validator_service_pins_build_and_runtime_contract() -> None:
    ...
    assert validator["stop_grace_period"] == "155m"
    assert validator["environment"]["VALIDATOR_SWEEP_SECONDS"] == "30"
    assert validator["environment"]["VALIDATOR_DITTOBENCH_TIMEOUT_SECONDS"] == "8400"
    wrapper = COMPOSE_WRAPPER_PATH.read_text()
...
def test_run_cap_stop_grace_and_updater_drains_are_coherent() -> None:
    ...
    # A direct stop and either updater can wait out one complete 150-minute
    # ticket plus five minutes for the validator's signed terminal report.
    # Keep at least 15 minutes over the 140-minute productive-run cap.
    assert stop_grace_seconds == 9_300
    assert stop_grace_seconds >= run_cap_seconds + 15 * 60

    source_wrapper = COMPOSE_WRAPPER_PATH.read_text()
    assert (
        source_wrapper.count("DITTO_VALIDATOR_COMPOSE_DRAIN_TIMEOUT_SECONDS:-9300") == 2
    )
    stack_updater = STACK_UPDATER_PATH.read_text()
    assert "setting VALIDATOR_AUTO_UPDATE_DRAIN_TIMEOUT_SECONDS 9300" in stack_updater
