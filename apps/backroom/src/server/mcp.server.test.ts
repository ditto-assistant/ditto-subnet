...
describe('Backroom MCP tools', () => {
  ...
  it('publishes every Backroom operation with MCP safety annotations', async () => {
    ...
    // the measured catalog just above 142 KB. Infra-retry, ordinary source
    // review queue-age SLO, hosted-inference taxonomy, reopened-hold reason,
    // and three process-key tools add bounded entries. Detailed canary
    // procedure stays in tool help.
    ...
    expect(descriptions.reduce((total, value) => total + value.length, 0)).toBeLessThanOrEqual(
      ...
      // reopened-hold reason, and three process-key summaries.
      28_000,
    )
