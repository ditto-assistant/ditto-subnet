...
export function createBackroomMcpServer(props: McpGrantProps) {
  ...
  registerTool(
    'evict_live_validator_leases',
    {
      ...
      description:
        'The operator escape hatch for a submission that is starving the validator fleet. Force-releases every validator ticket this submission currently holds, so those slots return to the pool on the validators\' next poll instead of running out their full 150-minute lease, and stops further assignment for the current benchmark era. ' +
        'Use it when one submission hangs during evaluation and reports nothing: quorum is 3, so every attempt holds 3 of the fleet\'s slots for 150 minutes and returns no score, while other submissions queue behind it. ' +
