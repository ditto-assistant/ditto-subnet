  ...
  registerTool(
    'set_queue_policy_settings',
    {
      ...
      description:
        ...
        'Scope: this block is the SOURCE-INTEGRITY branch only. It never gates copy/plagiarism enforcement — copy holds are opened by the duplicate-signal decision at score finalization, which does not read this policy and runs first — so mode=off leaves plagiarism detection fully armed. ' +
        'HAZARD when leaving enforce: observe and off stop new holds, but they also stop the mechanism that CLEARS the holds already open. A pending deferred hold normally auto-resolves when a screener re-claims the agent for its deep pass, and that re-claim is gated on mode=enforce. In any other mode no screener will pick those agents up again, so every already-pending hold freezes and its agent stays out of the emission-eligible ledger indefinitely, releasable only one at a time via resolve_ath_review. Nothing is auto-cleared, deliberately: a bulk clearance would write an unreasoned resolution onto each agent public audit record. Drain the open queue while still in enforce, then flip. ' +
