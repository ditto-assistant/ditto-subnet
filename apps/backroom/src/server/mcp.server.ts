...
export function createBackroomMcpServer(props: McpGrantProps) {
  ...
  registerTool(
    'set_inference_concurrency_settings',
    {
      ...
      description:
        'Apply the complete hosted-inference and v10 runtime policy with expectedRevision, reason, and "APPLY INFERENCE CONCURRENCY SETTINGS". benchmark_runtime controls case_concurrency (1-64, default 4) and an off/shadow 0-5000ms delay range. Concurrent /run uses the session URL. Shadow never changes scores. ' +
