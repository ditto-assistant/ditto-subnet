...
export function createBackroomMcpServer(props: McpGrantProps) {
  ...
  registerTool(
    'set_inference_concurrency_settings',
    {
      ...
      description:
        'Apply one append-only revision of the SN118 hosted inference admission policy live, with no platform redeploy; it reaches the Go relay within five seconds. Supply the COMPLETE policy — all eight numbers, every time. A revision stores the whole object and never a diff, so a field you leave out is NOT inherited from the current revision and is rejected here rather than quietly reset to a shipped default you did not choose. Also supply expectedRevision exactly as get_inference_concurrency_settings reports it as an optimistic-concurrency guard, an auditable reason of at least 8 characters, and the confirmation string "APPLY INFERENCE CONCURRENCY SETTINGS"; the signed-in operator is recorded as the actor. ' +
