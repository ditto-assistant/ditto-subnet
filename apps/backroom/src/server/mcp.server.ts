...
const MCP_CATALOG_DESCRIPTIONS: Record<string, string> = {
  ...
  resolve_ath_review:
    'Clear or reject one ATH hold with a public reason. Both require file:line evidenceReferences (policy v13); a reject also requires reasonCodes and bans. Writes a decision record. Requires backroom:write.',
...
export function createBackroomMcpServer(props: McpGrantProps) {
  ...
  registerTool(
    'resolve_ath_review',
    {
      ...
      description:
        'Clear or reject one ATH hold with an auditable public reason. Both REQUIRE evidenceReferences: at least one file:line citation into the reviewed source (policy v13 decision record; the platform refuses an uncited clear or reject with 422). Clearing restores the status held before a manual benchmark-overfit review; reasonCodes on it are optional published policy codes (e.g. I4.final_text_rewritten). Rejecting bans the submission and additionally REQUIRES reasonCodes, since it records a proven violation: every code must be a published proven-violation code (I1-I8 / S1-S3) from the policy-v13 "Required reason codes" catalog, e.g. I5.benchmark_semantic_compiler. Free text, invented codes, and the Q1/V1-V3 verification-failure codes are refused with 422 and change nothing; a policy version with no published catalog yet (v14 until its codes ship) refuses every reject. Both write a screening decision record readable with get_screening_decision_record. Requires backroom:write.',
