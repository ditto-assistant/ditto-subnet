...
async def resolve_reign_seed_anchor(
    ...
    *,
    ...
    """Create-or-read the reign's anchor and pin it once its height is finalized.

    ``None`` below the binding floor. Chain trouble while waiting is logged and
    leaves the row unpinned: the lane keeps serving catch-up and retries the
    pin on the next claim. It never fails the claim and never pins a
    non-finalized hash.
    """
    ...
    if row.anchor_block_hash is None:
        try:
            block_hash = await chain.get_finalized_block_hash(row.anchor_block)
