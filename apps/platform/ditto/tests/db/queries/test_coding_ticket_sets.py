...
async def test_ticket_set_racing_commit_replays_without_chain(monkeypatch) -> None:
    run_row_id = uuid4()
    ticket_set_id = uuid4()
    existing = [
        _ticket(
    ...
    class _RacingSession(_Session):
        def __init__(self) -> None:
            super().__init__(run=object())
            self.read_count = 0

        async def scalars(self, statement):
            del statement
            self.read_count += 1
            return [] if self.read_count == 1 else existing
