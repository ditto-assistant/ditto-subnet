...
| 9 | `test_includes_public_miner_facing_ath_review_queue` | Public ATH queue lives at `#/ath` (the `#/reviews` hash is now miner sign-in). Specs the ATH explainer copy, live list with cached-snapshot fallback, fan-out pagination over the public activity endpoint — and that nothing admin/auth leaks onto the public page. | src/pages/Reviews.test.tsx (endpoint path → src/lib/api.test.ts) |
