...
registerHooks({
  load(url, context, nextLoad) {
    if (url.startsWith('file:') && /\.(?:ts|mts|cts)$/.test(fileURLToPath(url))) {
      ...
      return { format: commonjs ? 'commonjs' : 'module', source: compiled.outputText, shortCircuit: true };
    }
...
function write(raw) {
  ...
  while (offset < raw.length) {
    try { offset += fs.writeSync(3, raw, offset, raw.length - offset); }
    catch (error) {
...
function encode(value) {
  try { return { kind: 'data', value: pack(value) }; }
  catch {
    if ((typeof value === 'object' && value !== null) || typeof value === 'function') {
      if (identities.has(value)) return { kind: 'reference', value: identities.get(value) };
    }
    if (references.size >= 4096) throw new Error('reference bound');
    const id = references.size + 1;
    references.set(id, value);
    if ((typeof value === 'object' && value !== null) || typeof value === 'function') identities.set(value, id);
...
async function execute(request) {
  ...
  if (typeof target.module === 'string') {
    if (!/^[A-Za-z][A-Za-z0-9_/-]*\.(?:js|mjs|cjs|ts|mts|cts)$/.test(target.module) ||
        target.module.split('/').some((part) => !part || part === '.' || part === '..')) {
      throw new Error('invalid module');
    }
    if (!modules.has(target.module)) {
      modules.set(target.module, await import(pathToFileURL('/workspace/' + target.module).href));
    }
  ...
  if (!Array.isArray(target.path) || target.path.length > 32) throw new Error('invalid path');
  ...
  if (request.operation === 'call' || request.operation === 'construct') {
    ...
    value = request.operation === 'call' ? Reflect.apply(value, receiver, args) : Reflect.construct(value, args);
...
async function main() {
  ...
  for await (const chunk of process.stdin) {
    ...
    if (typeof request.id !== 'string' || !/^[0-9a-f]{32}$/.test(request.id)) process.exit(1);
    let result;
    try { result = await execute(request); }
    catch { result = { kind: 'failure' }; }
