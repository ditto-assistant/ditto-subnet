...
if (process.argv[2] === 'child') {
  ...
  const counts = new Map(
    before.map((tid) => [
      ...
      Number(
        fs
          .readFileSync(`/proc/self/task/${tid}/status`, 'utf8')
  ...
  for (const tid of before) {
    ...
    assert.equal(
      Number(status.match(/^Seccomp_filters:\s+(\d+)$/m)[1]),
      counts.get(tid) + 1,
    );
  }
  ...
  assert.throws(() => fs.readFileSync('/run/dittobench-grader/secret'), {
    code: 'EACCES',
  });
  ...
  worker.on('message', ({ code, status }) => {
    ...
    assert.equal(
      Number(status.match(/^Seccomp_filters:\s+(\d+)$/m)[1]),
      counts.get(String(process.pid)) + 1,
    );
  });
