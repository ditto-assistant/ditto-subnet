...
usage() {
  cat <<'EOF'
Usage: scripts/targon-smoke.sh <inventory|list|rootless-probe|buildkit-probe|dind-probe|kaniko-probe|agent-probe|runtime-probe|vm-probe> [options]
...
case "${command}" in
  inventory)
    PYTHONPATH="${REPO_ROOT}" python3 -m screener_capacity.targon_cli \
      inventory "$@"
    ;;
  list|rootless-probe|buildkit-probe|dind-probe|kaniko-probe|agent-probe|runtime-probe|vm-probe)
