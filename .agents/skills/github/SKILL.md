...
Only after the user separately authorizes the merge. `gh stack` has **no** merge command, and a GitHub-linked stack rejects both `gh pr merge` (GraphQL) and the synchronous REST `pulls/{n}/merge` — each returns *"must be merged using the asynchronous merge REST API."* Merge through the async endpoint:

```bash
gh api -X PUT repos/<owner>/<repo>/pulls/<number>/merge-async \
  -f merge_method=squash \
  -f merge_action=default \
  -f commit_title='<conventional squash subject> (#<number>)'
```

It enqueues and returns `{"status":"pending",...}`; the merge runs in the background, so poll `gh pr view <number> --json state` until `MERGED`. It fails closed with `"Pull request is in draft"` — mark the layer ready first (`gh pr ready <number>`). `merge_method` also accepts `merge`/`rebase`; `merge_action` accepts `default`/`direct_merge`/`merge_queue`. The squash subject comes from `commit_title` (else the PR title) — keep it a valid semantic-release type so per-component releases fire.

**Merge the whole stack in one action — preferred and faster.** A stack merge is atomic and cascading: `merge-async` on any layer merges every unmerged PR up to *and including* that one. So mark **all** layers ready, then issue a **single** `merge-async` on the **top** PR — the entire stack lands (or is queued) atomically. Do not merge PR-by-PR unless you deliberately want to land only lower layers.

**Landing part of a stack** (one layer at a time): merge the bottom PR, then `gh stack sync --remote origin` rebases and retargets the upper PRs onto `main`. Sync **re-drafts** them, so `gh pr ready` again before the next `merge-async`. A squash-merged lower layer gets a new SHA on `main`, so this sync is required to drop the now-duplicated commits from the upper PRs.
