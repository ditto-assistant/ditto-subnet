...
| Multi-validator (k=3 + median-of-3) | **CODE DONE, deployment pending** | Platform implements k=3 leased tickets (`validator_tickets`), one signed score row per `(agent, validator)`, and median-at-quorum finalization; validators fold the median-aggregated ledger. Gap is running >=3 independent validators (only the owner runs today, so agents currently get one score). |
