...
| Validator scoring ticket | The first failed/expired lease exhausts its base budget; infrastructure grants do not wake it. A provider-circuit park refunds one resume per ticket; a resumed lease parked again is charged | `get_validation_retry`, `retry_validator_evaluation`, and `batch_retry_validator_evaluation` grant exactly one future lease per selected exhausted ticket; while the provider circuit is open, or a provider-parked slot is inside the recovery window, the row recommends waiting rather than a grant |
