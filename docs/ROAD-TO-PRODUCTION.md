...
| C-VERIFY | **Verifiable / replicable scoring** | **DECISION (path identified)** | For trustless scoring a validator must run its OWN scoring engine, not call a shared one. The dataset generator + per-case grader are already public and deterministic (`dittobench-datagen`), and the validator re-derives the on-chain seed (P2) and verifies the tarball sha256, so the dataset + per-case grades are independently reproducible. The GAP: the composite scorer (0.5/0.5 + canary/metamorphic/efficiency gates, `dittobench-api/internal/scorer`) and the sandbox runner + model-relay + egress-proxy live in the PRIVATE dittobench-api, so a third party can neither run the engine nor recompute the composite from the public per-case results. Fix is C-OPEN. No secrets block it: no answer key is hardcoded (regenerated per seed) and no keys are in source (env-only). |
| C-OPEN | **Open-source the dittobench-api scoring path** | **TODO (independence prerequisite)** | Publish the validator scoring engine so any staked validator runs its own and anyone can recompute + audit a composite: `internal/scorer` (composite + gates), `internal/sandbox` + `internal/runner` (build/run the miner tarball), `cmd/model-relay` + `cmd/egress-proxy` (the model + egress lock), `cmd/dittobench-api` (the `/v1/score` orchestration), `internal/netguard`. The generator + grader (`dittobench-datagen`) are already public. Nothing to redact (dataset regenerated from the on-chain seed; keys env-only). This is the concrete prerequisite for trustless independent validators (standard Bittensor: every validator evaluates independently and Yuma consensus clips weight-copiers). Until it lands, only the team can run a validator and the composite is not third-party-verifiable. |
...
| O-VAL | Third-party validator onboarding | **PARTIAL** | Onboarding kit shipped: `docs/VALIDATOR-ONBOARDING.md` rewritten for the k=3 model, `scripts/run-validator.sh` + `scripts/validator.env.example`, and a localnet three-validator recipe. The k=3 queue flow is implemented (F-MV). Blocking gap for genuine third parties: a validator must run its OWN dittobench-api scoring engine, which is still private (C-OPEN). Until C-OPEN, onboarding is limited to operators the team gives the private image. |
...
1. **Trust model** (owner-run scoring today vs permissionless-verifiable): the
   concrete lever is C-OPEN, open-sourcing the dittobench-api scoring path so any
   validator runs its own engine and the composite is third-party-verifiable.
   Decide whether to open it for the launch (full independence) or fast-follow
   with team-run k=3 validators first.
