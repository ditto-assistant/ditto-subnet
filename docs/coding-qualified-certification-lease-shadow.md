...
4. No in-flight certification lease exists, and no certification result
   (receipt) is still valid on the Platform database clock, for the same
   `(agent, artifact, screened image, benchmark, coding contract)` identity.
   A completed lease stays terminal, but after its result expires the same
   exact identity may renew under the operator allowlist and attempt budget.
...
artifact only, for the validity window of its receipt. Control-plane or sandbox infrastructure failure leaves a typed
...
2. Validator integration that claims one lease, fetches a lease-bound
   screened-image capability, launches the screened image in the existing
   hardened sandbox, runs `codingcertifier`, and revokes every route.
